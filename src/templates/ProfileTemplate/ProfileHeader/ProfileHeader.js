...
export default function ProfileHeader() {
  ...
  const renderUserInfo = () => {
    return (
      <>
        <h1 className="mb-0 mr-4 text-xl">Hello Harry!</h1>
        <button onClick={() => {
  ...
  const renderLogin = () => {
    return (
      <button className="text-lg text-black font-bold block mt-0 text-left duration-300 hover:text-white lg:hidden px-4 py-2 rounded lg:bg-green-400 mr-2 group hover:bg-yellow-400">
        <i className="fa fa-sign-out-alt lg:hidden inline"></i> Log out
      </button>
    )
  }
  ...
  return (
    <nav className="bg-gradient-to-r from-violet-500 to-cyan-500 flex items-center justify-between flex-wrap py-6 px-16 text-black">
      <div onClick={() => { navigate("/profile", { replace: false }) }} className="cursor-pointer flex items-center flex-no-shrink text-white mr-6">
      ...
      <div className={`w-full block flex-grow lg:flex lg:items-center lg:w-auto mt-4 lg:mt-0 ${navbarOpen ? "" : "hidden"}`}>
        <div className="text-sm lg:flex-grow flex lg:block flex-col">
          <button onClick={() => {
            ...
            <i className="fa fa-home lg:hidden inline"></i> Profile
          </button>
